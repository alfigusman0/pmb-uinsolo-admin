...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTabel').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url(); ?>setting/bobot-jurusan/jsondatatable",
				"type": "POST",
				"data": function(data) {
    ...
    $.ajax({
			url : "<?php echo base_url(); ?>setting/bobot-jurusan/get/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(response)
			{
				$('[name="ids_bobot_jurusan"]').val(response.data.ids_bobot_jurusan);
				$('[name="kode_jurusan"]').val(response.data.kode_jurusan);
				$('[name="tpa"]').val(response.data.tpa);
				$('[name="ips"]').val(response.data.ips);
				$('[name="ipa"]').val(response.data.ipa);
				$('[name="btq"]').val(response.data.btq);
				$('[name="tkd"]').val(response.data.tkd);
				$('[name="keislaman"]').val(response.data.keislaman);
				$('[name="bhs_arab"]').val(response.data.bhs_arab);
				$('[name="bhs_inggris"]').val(response.data.bhs_inggris);
				$('[name="bhs_indonesia"]').val(response.data.bhs_indonesia);
				$('[name="pembagi"]').val(response.data.pembagi);
	...
	function save()
	{
		...
		var url;
		if(save_method == 'add') {
			url = "<?php echo base_url('setting/bobot-jurusan/add')?>";
		} else {
			url = "<?php echo base_url('setting/bobot-jurusan/update')?>";
		}
	...
	function delete_data(id)
	{
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url : "<?php echo base_url('setting/bobot-jurusan/delete/')?>"+id,
					type: "POST",
					dataType: "JSON",
					success: function(response)
					{
