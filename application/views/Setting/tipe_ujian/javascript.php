...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTabel').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url(); ?>setting/tipe-ujian/jsondatatable",
				"type": "POST",
				"data": function(data) {
					data.tipe_ujian = $('#tipe_ujian').val();
				}
			},
	...
	function add_data() {
		...
		$('.modal-title').text('Tambah Tipe Ujian'); // Set Title to Bootstrap modal title
		$('[name="status_jadwal"][value="YA"]').prop("checked", true);
	...
	function edit_data(id) {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>setting/tipe-ujian/get/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(response) {
				...
				if (response.data.status_jadwal == 'TIDAK') {
					$('[name="status_jadwal"][value="TIDAK"]').prop("checked", true);
				} else {
					$('[name="status_jadwal"][value="YA"]').prop("checked", true);
				}
				$('[name="ids_tipe_ujian"]').val(response.data.ids_tipe_ujian);
				$('[name="ids_program"]').val(response.data.ids_program);
				$('[name="tipe_ujian"]').val(response.data.tipe_ujian);
				$('[name="kode"]').val(response.data.kode);
				$('[name="quota"]').val(response.data.quota);
				$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
				$('.modal-title').text('Edit Tipe Ujian'); // Set title to Bootstrap modal title
			},
	...
	function save() {
		...
		var url;
		if (save_method == 'add') {
			url = "<?php echo base_url('setting/tipe-ujian/add') ?>";
		} else {
			url = "<?php echo base_url('setting/tipe-ujian/update') ?>";
		}
	...
	function delete_data(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "<?php echo base_url('setting/tipe-ujian/delete/') ?>" + id,
					type: "POST",
					dataType: "JSON",
					success: function(response) {
