...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTabel').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url(); ?>setting/slider/jsondatatable",
				"type": "POST",
				"data": function(data) {
    ...
    $.ajax({
			url : "<?php echo base_url(); ?>setting/slider/get/"+id,
			type: "GET",
			dataType: "JSON",
			success: function(response)
			{
				...
				$('[name="ids_slider"]').val(response.data.ids_slider);
				$('[name="konten"]').val(response.data.konten);
	...
	function save()
	{
		...
		var url;
		if(save_method == 'add') {
			url = "<?php echo base_url('setting/slider/add')?>";
		} else {
			url = "<?php echo base_url('setting/slider/update')?>";
		}
		var formData = new FormData();
		formData.append('gambar', $('[name="gambar"]')[0].files[0]);
		formData.append('ids_slider', $('#ids_slider').val());
		formData.append('konten', $('#konten').val());
		formData.append('status', $('input[type="radio"][name="status"]:checked').val());
	...
	function delete_data(id)
	{
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url : "<?php echo base_url('setting/slider/delete/')?>"+id,
					type: "POST",
					dataType: "JSON",
					success: function(response)
					{
