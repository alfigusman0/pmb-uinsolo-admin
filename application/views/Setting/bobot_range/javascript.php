...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTabel').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url(); ?>setting/bobot-range/jsondatatable",
				"type": "POST",
				"data": function(data) {
	...
	function add_data()
	{
			...
			$('.modal-title').text('Tambah Bobot Range UKT'); // Set Title to Bootstrap modal title
			$('[name="ids_jalur_masuk"]').attr('disabled', false);
	}
    ...
    $.ajax({
			url : "<?php echo base_url(); ?>setting/bobot-range/get/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(response)
			{
				$('[name="ids_bobot_range_ukt"]').val(response.data.ids_bobot_range_ukt);
				$('[name="kategori"]').val(response.data.kategori);
				$('[name="nilai_min"]').val(response.data.nilai_min);
				$('[name="nilai_max"]').val(response.data.nilai_max);
				$('[name="ids_jalur_masuk"]').val(response.data.ids_jalur_masuk);
				$('[name="tahun"]').val(response.data.tahun);
				$('[name="ids_jalur_masuk"]').attr('disabled', true);
				$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
				$('.modal-title').text('Edit Bobot Range UKT'); // Set title to Bootstrap modal title
			},
	...
	function save()
	{
		...
		var url;
		if(save_method == 'add') {
			url = "<?php echo base_url('setting/bobot-range/add')?>";
		} else {
			url = "<?php echo base_url('setting/bobot-range/update')?>";
		}
	...
	function delete_data(id)
	{
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url : "<?php echo base_url('setting/bobot-range/delete/')?>"+id,
					type: "POST",
					dataType: "JSON",
					success: function(response)
					{
	...
	function generate()
	{
			...
			$('.modal-title').text('Generate Range UKT'); // Set Title to Bootstrap modal title
	}
	...
	function act_generate()
	{
		...
		// ajax adding data to database
		$.ajax({
			url : "<?php echo base_url('setting/bobot-range/generate')?>",
			type: "POST",
	...
	function act_simpan_setting()
	{
		...
		// ajax adding data to database
		$.ajax({
			url : "<?php echo base_url('setting/bobot-range/simpan')?>",
			type: "POST",
			dataType: "JSON",
			success: function(response)
			{
