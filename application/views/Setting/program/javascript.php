...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#dataTabel').DataTable({
			...
			"ajax": {
				"url": "<?php echo base_url(); ?>setting/program/jsondatatable",
				"type": "POST",
				"data": function(data) {
    ...
    $.ajax({
			url : "<?php echo base_url(); ?>setting/program/get/",
			data: {
				ids_program: id
			},
			type: "GET",
			...
			{
				if(response.data[0].status == 'TIDAK'){
					$('[name="status"][value="TIDAK"]').prop("checked", true);
				}else{
					$('[name="status"][value="YA"]').prop("checked", true);
				}
				$('[name="ids_program"]').val(response.data[0].ids_program);
				$('[name="kode_program"]').val(response.data[0].kode_program);
				$('[name="program"]').val(response.data[0].program);
				$('[name="jenjang"]').val(response.data[0].jenjang);
				$('[name="kelas"]').val(response.data[0].kelas);
				if(response.data[0].jenjang !== null){
					$.each(response.data[0].jenjang.split(","), function(i,e){
						$("#jenjang_select option[value='" + e + "']").prop("selected", true);
					});
				}
				if(response.data[0].kelas !== null){
					$.each(response.data[0].kelas.split(","), function(i,e){
						$("#kelas_select option[value='" + e + "']").prop("selected", true);
					});
				}
	...
	function save()
	{
		...
		var url;
		if(save_method == 'add') {
			url = "<?php echo base_url('setting/program/add')?>";
		} else {
			url = "<?php echo base_url('setting/program/update')?>";
		}
		
		$('#jenjang').val($('#jenjang_select').val().toString())
		$('#kelas').val($('#kelas_select').val().toString())
	...
	function delete_data(id)
	{
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url : "<?php echo base_url('setting/program/delete/')?>"+id,
					type: "POST",
					dataType: "JSON",
					success: function(response)
					{
