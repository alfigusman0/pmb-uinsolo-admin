...
class SubDayaTampung extends CI_Controller
{
    ...
    function index()
    {
        $hak_akses = $this->master->read('hak-akses/?ids_level=' . $this->jwt->ids_level . '&ids_modul=70&aksi_hak_akses=import');
        if ($hak_akses->code == 200) {
            $data = array(
                'title' => 'Update Sub Daya Tampung (Update) | ' . $_ENV['APPLICATION_NAME'],
                'content' => 'Import/Setting/sub_daya_tampung/content',
                'css' => 'Import/Setting/sub_daya_tampung/css',
                'javascript' => 'Import/Setting/sub_daya_tampung/javascript',
                'modal' => 'Import/Setting/sub_daya_tampung/modal',
        ...
        if ($hak_akses->code == 200) {
            ...
            if (!$this->upload->do_upload()) {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $daya_tampung = $sheet->getCellByColumnAndRow(3, $row)->getValue();
                    $rules = array(
                ...
                redirect('setting/import/sub-daya-tampung');
            }
        } else {
            $this->session->set_flashdata('message', 'Hak akses ditolak.');
            $this->session->set_flashdata('type_message', 'danger');
