...
class PenguranganKuota extends CI_Controller
{
    var $jwt = null;
    ...
    function index()
    {
        ...
        if ($hak_akses->code == 200) {
            ...
            $data = array(
                'title' => 'Import Setting Pengurangan Kuota | ' . $_ENV['APPLICATION_NAME'],
                'content' => 'Import/Setting/pengurangan_kuota/content',
                'css' => 'Import/Setting/pengurangan_kuota/css',
                'javascript' => 'Import/Setting/pengurangan_kuota/javascript',
                'modal' => 'Import/Setting/pengurangan_kuota/modal',
    ...
    function Import()
    {
        ...
        if ($hak_akses->code == 200) {
            ...
            if (!$this->upload->do_upload()) {
                ...
                $this->session->set_flashdata('type_message', 'danger');
                redirect('import/setting/pengurangan-kuota');
            } else {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $kuota_kurang = $sheet->getCellByColumnAndRow(3, $row)->getValue();
                    $rules = array(
                    ...
                    if ($cek->num_rows() > 0) {
                        ...
                        if (!$fb['status']) {
                            ...
                            $error++;
                            $error_message .= "Jurusan $jurusan ($kode_jurusan / " . $this->input->post('ids_jalur_masuk') . ") : $fb->message <br>";
                        }
                    }
                }
                ...
                redirect('setting/import/pengurangan-kuota');
            }
        } else {
            $this->session->set_flashdata('message', 'Hak akses ditolak.');
            $this->session->set_flashdata('type_message', 'danger');
