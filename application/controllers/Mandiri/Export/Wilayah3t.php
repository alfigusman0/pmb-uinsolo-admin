...
class Wilayah3t extends CI_Controller
{
    var $jwt = null;
    ...
    function index()
    {
        ...
        if ($hak_akses->code == 200) {
            ...
            $data = array(
                'title'         => 'Export Wilayah 3T | ' . $_ENV['APPLICATION_NAME'],
                'content'       => 'Export/Mandiri/wilayah3t/content',
                'css'           => 'Export/Mandiri/wilayah3t/css',
                'javascript'    => 'Export/Mandiri/wilayah3t/javascript',
                'modal'         => 'Export/Mandiri/wilayah3t/modal',
        ...
        if ($hak_akses->code == 200) {
            ...
            if ($viewpFormulir->num_rows() > 0) {
                ...
                foreach ($viewpFormulir->result() as $value) {
                    ...
                    if($viewpRumah->wilayah_3t == 'TIDAK'){
                        continue;
                    }
                ...
                $filename = urlencode("Wilayah_3T_" . $this->input->post('tahun') . "_" . $this->input->post('ids_program') . "_" . $this->input->post('ids_tipe_ujian') . "_" . date("Y_m_d_H_i_s") . ".xlsx");
                ...
                $this->session->set_flashdata('type_message', 'danger');
                redirect('mandiri/export/wilayah3t');
            }
        } else {
            $this->session->set_flashdata('message', 'Hak akses ditolak.');
            $this->session->set_flashdata('type_message', 'danger');
