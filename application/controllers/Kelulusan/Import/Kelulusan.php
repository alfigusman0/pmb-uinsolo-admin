...
class Kelulusan extends CI_Controller
{
    ...
    function index()
    {
        $hak_akses = $this->master->read('hak-akses/?ids_level=' . $this->jwt->ids_level . '&ids_modul=91&aksi_hak_akses=import');
        if ($hak_akses->code == 200) {
            ...
            $data = array(
                'title' => 'Import Kelulusan | ' . $_ENV['APPLICATION_NAME'],
                'content'   => 'Import/Kelulusan/kelulusan/content',
                'css'       => 'Import/Kelulusan/kelulusan/css',
                'javascript' => 'Import/Kelulusan/kelulusan/javascript',
                'modal'     => 'Import/Kelulusan/kelulusan/modal',
    ...
    function Import()
    {
        $hak_akses = $this->master->read('hak-akses/?ids_level=' . $this->jwt->ids_level . '&ids_modul=93&aksi_hak_akses=import');
        if ($hak_akses->code == 200) {
            $config = array(
                ...
                'max_size' => 51200,
                'overwrite' => TRUE,
                'file_name' => 'Kelulusan_' . date('Y') . '_' . date('HisdmY'),
            );
                                ...
                                if ($tbsDayaTampung->kuota > 0) {
                                    ...
                                    if ($tbpFormulir->ids_tipe_ujian == 1) {
                                        ...
                                        if ($tbpKelulusan->lulus != 'YA') {
                                            if ($tbpKelulusan->total >= $tbsDayaTampung->grade) {
                                                // Update Kelulusan
                                                $rules = array(
                                                    'where' => array(
                                                        'idp_formulir' => $tbpFormulir->idp_formulir,
