...
class Bebas extends CI_Controller
{
    ...
    function index()
    {
        ...
        if ($hak_akses->code == 200) {
            ...
            $data = array(
                'title' => 'Import Lulus Bebas | ' . $_ENV['APPLICATION_NAME'],
                'content'   => 'Import/Kelulusan/bebas/content',
                'css'       => 'Import/Kelulusan/bebas/css',
                'javascript' => 'Import/Kelulusan/bebas/javascript',
                'modal'     => 'Import/Kelulusan/bebas/modal',
    ...
    function Import()
    {
        ...
        if ($hak_akses->code == 200) {
            ...
            if (!$this->upload->do_upload()) {
                ...
                $this->session->set_flashdata('type_message', 'danger');
                redirect('kelulusan/import/bebas');
            } else {
                ...
                try {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        $nama_penitip = $sheet->getCellByColumnAndRow(3, $row)->getValue();
                        ...
                        if ($check > 0) {
                            ...
                            $tbpPilihan = $this->Tbp_pilihan->search($rules)->result();
                            foreach ($tbpPilihan as $bebas) {
                                // Cek Kuota Daya Tampung
                                $rules = array(
                                    'database'    => null,
                                    'select'    => null,
                                    'where'     => array(
                                        'kode_jurusan' => $bebas->kode_jurusan,
