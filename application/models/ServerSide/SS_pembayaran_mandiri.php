...
class SS_pembayaran_mandiri extends CI_Model
{
	protected $table = 'viewp_pembayaran'; // Nama Tabel
	...
	private function _get_datatables_query()
	{
		...
		$this->_apply_filter('idp_formulir', 'idp_formulir', $post, 'like', true);
	 ...
	 */
	private function _apply_filter($field, $db_field, $post, $type, $substring = false)
	{
		if (!empty($post[$field])) {
			$value = $post[$field];
			if ($substring) {
				$value = substr($value, 4); // Apply substring if needed
			}
			if ($type === 'like') {
